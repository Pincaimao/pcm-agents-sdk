...
| `customInputs`         | `custom-inputs`          | 自定义输入参数，传入customInputs.job_info时，会隐藏JD输入区域<br> 传入customInputs.file_urls时，会隐藏简历上传区域。<br> 传入customInputs.file_urls和customInputs.job_info时，会直接开始聊天。<br> | `{ [x: string]: string; }` | `{}`        |
...
| `modalTitle`           | `modal-title`            | 模态框标题                                                                                                                                                | `string`                   | `'面试报告'`    |
...
graph TD;
  ...
  style pcm-msbg-modal fill:#f9f,stroke:#333,stroke-width:4px
```
