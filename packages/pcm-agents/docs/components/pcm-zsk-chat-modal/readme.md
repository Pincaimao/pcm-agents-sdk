...
| `customInputs`            | `custom-inputs`            | 自定义智能体inputs输入参数:<br> 1. show_suggested_questions: 是否显示推荐问题<br> 支持字符串格式（将被解析为JSON）或对象格式 | `string \| { [x: string]: string; }` | `{     show_suggested_questions: 'false',   }` |
