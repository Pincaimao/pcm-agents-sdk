...
| `customInputs`   | --                | 自定义输入参数，传入plan_type则可以指定规划类型，可传入"长期规划"、"转行建议"、"晋升路径" 例如：    zyghModal.customInputs = {      plan_type: "转行建议" }; | `{ [key: string]: any; }` | `{}`        |
...
| `planningComplete`  | 当聊天完成时触发                | `CustomEvent<{ conversation_id: string; plan_type: CareerPlanType; }>`                     |
