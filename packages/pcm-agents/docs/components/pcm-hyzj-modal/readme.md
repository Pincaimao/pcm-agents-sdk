...
| `customInputs`       | --                | 自定义输入参数，传入job_info时，会隐藏JD输入区域  hyzjModal.customInputs = {      job_info: "负责市场营销策略制定与执行；开展市场调研，分析竞争对手情况；策划并执行线上线下营销活动；" }; | `{ [x: string]: any; }` | `{}`        |
...
| `modalTitle`         | `modal-title`     | 模态框标题                                                                                                                      | `string`                | `'会议总结助手'`  |
...
graph TD;
  ...
  style pcm-hyzj-modal fill:#f9f,stroke:#333,stroke-width:4px
```
