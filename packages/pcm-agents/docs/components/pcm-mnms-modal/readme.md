...
| `customInputs`        | `custom-inputs`         | 自定义输入参数，传入customInputs.job_info时，会隐藏JD输入区域 传入customInputs.file_url时，会隐藏简历上传区域 传入customInputs.file_url和customInputs.job_info时，会直接开始聊天 | `{ [x: string]: any; }` | `{}`        |
